...
END as CUSTOMER_CATEGORY
 ...
 select PRODUCT_ID, PRODUCT_DESC, PRODUCT_QUANTITY_AVAIL, PRODUCT_PRICE, PRODUCT_PRICE*PRODUCT_QUANTITY_AVAIL as INVENTORY_VALUE,
 CASE
 WHEN PRODUCT_PRICE > 20000 THEN PRODUCT_PRICE - 0.2*PRODUCT_PRICE
 WHEN PRODUCT_PRICE <= 20000 AND PRODUCT_PRICE > 10000 THEN PRODUCT_PRICE - 0.15*PRODUCT_PRICE
 WHEN PRODUCT_PRICE <= 10000 THEN PRODUCT_PRICE - 0.1*PRODUCT_PRICE
 END as NEW_PRICE 
 from PRODUCT
 ...
 select A.PRODUCT_CLASS_CODE, B.PRODUCT_CLASS_DESC, COUNT(A.PRODUCT_ID), SUM(A.PRODUCT_PRICE*A.PRODUCT_QUANTITY_AVAIL) as INVENTORY_VALUE from PRODUCT as A, PRODUCT_CLASS as B
 WHERE A.PRODUCT_CLASS_CODE=B.PRODUCT_CLASS_CODE 
 ...
 select o.CUSTOMER_ID, CONCAT(o.CUSTOMER_FNAME, ' ', o.CUSTOMER_LNAME) as CUSTOMER_NAME, o.CUSTOMER_EMAIL, o.CUSTOMER_PHONE, a.COUNTRY from ONLINE_CUSTOMER as o, ADDRESS as a
 where o.ADDRESS_ID = a.ADDRESS_ID and o.CUSTOMER_ID in
 (select CUSTOMER_ID  from ORDER_HEADER where CUSTOMER_ID IN (select CUSTOMER_ID from ORDER_HEADER where ORDER_STATUS = 'Cancelled')
 ...
 select S.SHIPPER_NAME, A.City, COUNT(DISTINCT(O.Customer_ID)) as NUMBER_OF_CUSTOMERS, COUNT(DISTINCT(O.Order_ID)) as NUMBER_OF_ORDERS from 
 SHIPPER S JOIN ORDER_HEADER O
 ON S.SHIPPER_ID = O.SHIPPER_ID
 JOIN ONLINE_CUSTOMER OL
 ON O.CUSTOMER_ID = OL.CUSTOMER_ID
 JOIN ADDRESS A
 ON OL.ADDRESS_ID = A.ADDRESS_ID
 where S.SHIPPER_NAME = 'DHL' and O.Order_Status = 'Shipped'
 ...
 select P.PRODUCT_ID, P.PRODUCT_DESC, P.PRODUCT_QUANTITY_AVAIL, SUM(O.PRODUCT_QUANTITY) as PRODUCT_QUANTITY_SOLD, 
 CASE 
 WHEN (PRODUCT_CLASS_CODE=2050 or PRODUCT_CLASS_CODE=2053) and SUM(O.PRODUCT_QUANTITY) = 0 THEN 'No Sales in past, give discount to reduce inventory'
 WHEN (PRODUCT_CLASS_CODE=2050 or PRODUCT_CLASS_CODE=2053) and P.PRODUCT_QUANTITY_AVAIL < 0.1*SUM(O.PRODUCT_QUANTITY) THEN 'Low inventory, need to add inventory'
 WHEN (PRODUCT_CLASS_CODE=2050 or PRODUCT_CLASS_CODE=2053) and P.PRODUCT_QUANTITY_AVAIL >= 0.1*SUM(O.PRODUCT_QUANTITY) AND P.PRODUCT_QUANTITY_AVAIL < 0.5*SUM(O.PRODUCT_QUANTITY) THEN 'Medium inventory, need to add some inventory'
 WHEN (PRODUCT_CLASS_CODE=2050 or PRODUCT_CLASS_CODE=2053) and P.PRODUCT_QUANTITY_AVAIL >= 0.5*SUM(O.PRODUCT_QUANTITY) THEN 'Sufficient inventory'
 WHEN (PRODUCT_CLASS_CODE=2055 or PRODUCT_CLASS_CODE=2057) and SUM(O.PRODUCT_QUANTITY) = 0 THEN 'No Sales in past, give discount to reduce inventory'
 WHEN (PRODUCT_CLASS_CODE=2055 or PRODUCT_CLASS_CODE=2057) and P.PRODUCT_QUANTITY_AVAIL < 0.2*SUM(O.PRODUCT_QUANTITY) THEN 'Low inventory, need to add inventory'
 WHEN (PRODUCT_CLASS_CODE=2055 or PRODUCT_CLASS_CODE=2057) and P.PRODUCT_QUANTITY_AVAIL >= 0.2*SUM(O.PRODUCT_QUANTITY) AND P.PRODUCT_QUANTITY_AVAIL < 0.6*SUM(O.PRODUCT_QUANTITY) THEN 'Medium inventory, need to add some inventory'
 WHEN (PRODUCT_CLASS_CODE=2055 or PRODUCT_CLASS_CODE=2057) and P.PRODUCT_QUANTITY_AVAIL >= 0.6*SUM(O.PRODUCT_QUANTITY) THEN 'Sufficient inventory'
 WHEN (PRODUCT_CLASS_CODE!=2050 and PRODUCT_CLASS_CODE!=2053 and PRODUCT_CLASS_CODE!=2055 and PRODUCT_CLASS_CODE!=2057) and SUM(O.PRODUCT_QUANTITY) = 0 THEN 'No Sales in past, give discount to reduce inventory'
 WHEN (PRODUCT_CLASS_CODE!=2050 and PRODUCT_CLASS_CODE!=2053 and PRODUCT_CLASS_CODE!=2055 and PRODUCT_CLASS_CODE!=2057) and P.PRODUCT_QUANTITY_AVAIL < 0.3*SUM(O.PRODUCT_QUANTITY) THEN 'Low inventory, need to add inventory'
 WHEN (PRODUCT_CLASS_CODE!=2050 and PRODUCT_CLASS_CODE!=2053 and PRODUCT_CLASS_CODE!=2055 and PRODUCT_CLASS_CODE!=2057) and P.PRODUCT_QUANTITY_AVAIL >= 0.3*SUM(O.PRODUCT_QUANTITY) AND P.PRODUCT_QUANTITY_AVAIL < 0.7*SUM(O.PRODUCT_QUANTITY) THEN 'Medium inventory, need to add some inventory'
 WHEN (PRODUCT_CLASS_CODE!=2050 and PRODUCT_CLASS_CODE!=2053 and PRODUCT_CLASS_CODE!=2055 and PRODUCT_CLASS_CODE!=2057) and P.PRODUCT_QUANTITY_AVAIL >= 0.7*SUM(O.PRODUCT_QUANTITY) THEN 'Sufficient inventory'
 END as INVENTORY_STATUS
 from PRODUCT as P, ORDER_ITEMS as O
 ...
 select o.ORDER_ID, SUM(o.PRODUCT_QUANTITY*p.LEN*p.WIDTH*P.HEIGHT) as ORDER_VOLUME from 
 ORDER_ITEMS as O JOIN PRODUCT as p ON o.PRODUCT_ID=p.PRODUCT_ID
 GROUP BY o.ORDER_ID
 HAVING SUM(o.PRODUCT_QUANTITY*p.LEN*p.WIDTH*P.HEIGHT) < (select LEN*WIDTH*HEIGHT from CARTON where CARTON_ID = 10)
 ORDER BY SUM(o.PRODUCT_QUANTITY*p.LEN*p.WIDTH*P.HEIGHT) DESC
 LIMIT 1;
 ...
 select OC.CUSTOMER_ID, CONCAT(OC.CUSTOMER_FNAME, ' ', OC.CUSTOMER_LNAME) as CUSTOMER_NAME, SUM(OI.PRODUCT_QUANTITY) AS TOTAL_QUANTITY, SUM(OI.PRODUCT_QUANTITY*P.PRODUCT_PRICE) as TOTAL_VALUE
 from ONLINE_CUSTOMER as OC JOIN ORDER_HEADER as OH ON OC.CUSTOMER_ID = OH.CUSTOMER_ID
 ...
 GROUP BY OC.CUSTOMER_ID;
  ...
  # Query 9
  # (Assumptions: Total Product Quantities are considered only restricted to the orders where PRODUCT 201 was ordered.)
  # (Total quantities apart from those orders can also be considered as per requirement but since the requirement is not explicitly specified, hence have taken this approach)
  # For example: Product 218 might have been ordered apart from those orders where Product 201 was ordered and total quantity would be different.
  # But here we have considered the quantity of Product 218 in orders where Product 201 was ordered.
  
  select P.PRODUCT_ID, P.PRODUCT_DESC, SUM(OI.PRODUCT_QUANTITY)
  from PRODUCT as P
  ...
  where OH.ORDER_ID in (select ORDER_ID from ORDER_ITEMS where PRODUCT_ID = 201) and P.PRODUCT_ID != 201 and A.city != 'Bangalore' and A.city != 'New Delhi'
  GROUP BY PRODUCT_ID
  ...
  select OI.ORDER_ID, OH.CUSTOMER_ID, CONCAT(OC.CUSTOMER_FNAME, ' ', OC.CUSTOMER_LNAME) as CUSTOMER_NAME, sum(OI.PRODUCT_QUANTITY)
  from ORDER_HEADER as OH JOIN ONLINE_CUSTOMER as OC on OH.CUSTOMER_ID = OC.CUSTOMER_ID
  JOIN ADDRESS as A ON OC.ADDRESS_ID = A.ADDRESS_ID
